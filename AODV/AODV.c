...
/**
AODV Init function initialises the AODV parameters.
*/
	...
	case NETWORK_OUT_EVENT:
		{
				...
				pstruEventDetails->pPacket->nControlDataType/100 != NW_PROTOCOL_AODV)
			{
				...
				NETSIM_IPAddress ip = AODV_DEV_IP(pstruEventDetails->nDeviceId);
			...
			else
			{
				...
				pstruEventDetails->pPacket->pstruNetworkData->szNextHopIp=AODV_DEV_IP_COPY(pstruEventDetails->pPacket->nReceiverId);
			}
		...
		default:
			{
				if(pstruEventDetails->pPacket->nDestinationId == pstruEventDetails->nDeviceId)
				{
					...
					AODV_UPDATE_ROUTE_TABLE(pstruEventDetails->pPacket->pstruNetworkData->szSourceIP,
				...
				else
				{
					...
					AODV_UPDATE_ROUTE_TABLE(pstruEventDetails->pPacket->pstruNetworkData->szDestIP,
						pstruEventDetails->dEventTime+AODV_ACTIVE_ROUTE_TIMEOUT);
...
*/
_declspec(dllexport) int fn_NetSim_AODV_Metrics(char* file)
{
	return fn_NetSim_AODV_Metrics_F(file);
}
/**
