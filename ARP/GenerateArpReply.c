...
//_declspec (dllexport) int fn_NetSim_Generate_ARP_Reply()
{
	...
	{	
		...
		pstruArpReplyPkt->sz_ar$sha = _strdup(DEVICE_INTERFACE(nDeviceId,nInterfaceId)->pstruMACLayer->szMacAddress);
		pstruArpReplyPkt->sz_ar$tpa = IP_COPY(pstruArpRequestPkt->sz_ar$spa);
		pstruArpReplyPkt->sz_ar$tha = _strdup(pstruArpRequestPkt->sz_ar$sha);			
		pstruArpReplyPkt->n_ar$op = ares_opSREPLY; // Set Opcode as REPLY 
		//Set the Ethernet header details
		pstruArpReplyPkt->szDestMac = _strdup(pstruArpRequestPkt->sz_ar$sha);
		pstruArpReplyPkt->szSrcMac = _strdup(DEVICE_INTERFACE(nDeviceId,nInterfaceId)->pstruMACLayer->szMacAddress);
		...
		pstruControlPacket->nDestinationId=pstruEventDetails->pPacket->nSourceId;
		...
		pstruControlPacket->pstruMacData->szSourceMac = _strdup(DEVICE_INTERFACE(nDeviceId,nInterfaceId)->pstruMACLayer->szMacAddress);
		pstruControlPacket->pstruMacData->szDestMac = _strdup(pstruArpRequestPkt->sz_ar$sha);					
		...
		// Increament the reply sent count		
		pstruArpVariables->pstruArpMetrics->nArpReplySentCount += 1;
		
		// Write to LogFile	
		szDestIPadd = DEVICE_NWADDRESS(nDeviceId,nInterfaceId); //_strdup(DEVICE_NWADDRESS(nDeviceId,nInterfaceId));
		szTargetMAC = pstruArpReplyPkt->sz_ar$sha; //_strdup(pstruArpReplyPkt->sz_ar$sha);			
		printf("ARP---ARP_ReplySent\tEventTime:%0.3lf\tSrcIP:%s\tMAC_Add:%s\tDestIP:%s \n",pstruEventDetails->dEventTime,szDestIPadd,szTargetMAC,szSrcIPadd);
