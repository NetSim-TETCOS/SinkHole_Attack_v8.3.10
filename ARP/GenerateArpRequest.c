...
int fn_NetSim_Generate_ARP_Request(NetSim_EVENTDETAILS *pstruEventDetails, struct stru_NetSim_Network *NETWORK)
{
	...
	szSubnetMaskAdd = DEVICE_INTERFACE(nDeviceId,nInterfaceId)->szSubnetMask;//_strdup(DEVICE_INTERFACE(nDeviceId,nInterfaceId)->szSubnetMask);
	...
	//Add the relevent fields for ARP REQUEST
	...
	pstruArpRequestPkt->sz_ar$sha= _strdup(DEVICE_INTERFACE(nDeviceId,nInterfaceId)->pstruMACLayer->szMacAddress);
	...
	pstruArpRequestPkt->szDestMac = _strdup("FFFFFFFFFFFF"); // ff-ff-ff-ff-ff-ff
	pstruArpRequestPkt->szSrcMac = _strdup(DEVICE_INTERFACE(nDeviceId,nInterfaceId)->pstruMACLayer->szMacAddress);
	...
	pstruControlPacket->nDestinationId = 0;
	...
	pstruControlPacket->pstruNetworkData->szDestIP = STR_TO_IP4("255.255.255.255");	
	...
	pstruControlPacket->pstruMacData->szDestMac = _strdup("FFFFFFFFFFFF");	
