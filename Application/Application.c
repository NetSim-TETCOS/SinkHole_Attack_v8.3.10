	...
	case APPLICATION_OUT_EVENT:
		{
			...
			if(pstruPacket)
			{
				...
				//Initiallize the aplication data
				...
				fn_NetSim_Add_DummyPayload(pstruPacket);
...
/** 
This function is used to genearate the packets for application 
*/
...
int fn_NetSim_Add_DummyPayload(NetSim_PACKET* packet)
{
	while(packet)
	{
		if(!packet->szPayload && packet->pstruAppData && packet->pstruAppData->dPacketSize && wireshark_flag)
		{
			...
			copy_payload(packet->szPayload->packet,packet,&size);
