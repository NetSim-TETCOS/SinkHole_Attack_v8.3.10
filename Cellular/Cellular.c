					...
					else
					{
						if(DEVICE_MACLAYER(pstruEventDetails->nDeviceId,pstruEventDetails->nInterfaceId)->nMacProtocolId == MAC_PROTOCOL_GSM)
						{
							...
							if(pstruEventDetails->pPacket->pstruAppData && pstruEventDetails->pPacket->pstruAppData->nApplicationId)
								flag = isCellularChannelAllocated(pstruEventDetails->pPacket->nDestinationId,1,pstruEventDetails->pPacket->pstruAppData->nApplicationId);
			...
			case CELLULAR_SUBEVENT(MAC_PROTOCOL_CDMA,Subevent_TxNextBurst):
				{
					...
					list[pstruEventDetails->pPacket->pstruAppData->nApplicationId][pstruEventDetails->pPacket->nSourceId-1][pstruEventDetails->pPacket->nDestinationId-1]=list[pstruEventDetails->pPacket->pstruAppData->nApplicationId][pstruEventDetails->pPacket->nSourceId-1][pstruEventDetails->pPacket->nDestinationId-1]->pstruNextPacket;
					pstruEventDetails->nEventType=PHYSICAL_OUT_EVENT;
...
int fn_NetSim_Cellular_ChannelResponse(NetSim_PACKET* packet)
{
	...
	else if(response->nAllocationFlag==0 && MSMac->nMSStatusFlag==Status_ChannelRequested)
	{
		...
		info->fn_BlockCall(appInfo[packet->pstruAppData->nApplicationId-1],packet->nSourceId,packet->nDestinationId,pstruEventDetails->dEventTime);
...
int fn_NetSim_Cellular_Msc_ProcessPacket()
{
	...
	NETSIM_ID nDestinationId=packet->nDestinationId;
	//Contact VLR list
...
int fn_NetSim_Cellular_MS_ProcessCallRequest()
{
	Cellular_MS_MAC* MSMac=DEVICE_MACVAR(pstruEventDetails->nDeviceId,pstruEventDetails->nInterfaceId);
	NetSim_PACKET* packet=pstruEventDetails->pPacket;
	NETSIM_ID nSouceId=packet->nDestinationId;
	...
	else
	{
		...
		while(channel)
		{
			if(channel->nAllocationFlag==1 && channel->nApplicationId==nApplicationId
				&& channel->nMSId == packet->nDestinationId)
			{
				gsmPacket->nTimeSlot=channel->nTimeSlot;
				break;
			}
...
int fn_NetSim_Cellular_MS_PhyOut()
{
	...
	double dStartTime;
	double dDataRate=DEVICE_PHYLAYER(pstruEventDetails->nDeviceId,pstruEventDetails->nInterfaceId)->pstruNetSimLinks->puniMedProp.pstruWirelessLink.dDataRateUp;
	packet->pstruPhyData->dArrivalTime=pstruEventDetails->dEventTime;
	if(pstruEventDetails->nProtocolId==MAC_PROTOCOL_GSM)
	...
	if(packet->nControlDataType==CELLULAR_PACKET_TYPE(pstruEventDetails->nProtocolId,PacketType_DropCall))
	{
		...
		MSMac->pstruPacketList[pstruEventDetails->nApplicationId][packet->nSourceId-1][packet->nDestinationId-1])
	{
	...
	{
		//Add call end event
		fn_NetSim_Cellular_SendCallend(nMSID,nMSInterface,packet->nDestinationId,pstruEventDetails->dEventTime);
	}
	return 1;
}
