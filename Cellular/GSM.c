...
int fn_NetSim_Cellular_AllocateChannel(NetSim_EVENTDETAILS* pstruEventDetails,NetSim_PACKET* packet)
{
	...
	if(MSMac->nMSStatusFlag!=Status_IDLE && MSMac->nMSStatusFlag != Status_CallEnd)
	{
		if(MSMac->nApplicationId!=nApplicationId)
		{
			...
			info->fn_BlockCall(appInfo[nApplicationId-1],packet->nSourceId,packet->nDestinationId,pstruEventDetails->dEventTime);
	...
	else
	{
		...
		requestData->nDestId=packet->nDestinationId;
...
int fn_NetSim_GSM_BS_PhyOut()
{
	...
	double dStartTime;
	double dDataRate=DEVICE_PHYLAYER(pstruEventDetails->nDeviceId,pstruEventDetails->nInterfaceId)->pstruNetSimLinks->puniMedProp.pstruWirelessLink.dDataRateDown;
