...
/** Stucture for uplink allocation information. */
...
/** Base statucture for 802.22 Phy */
struct stru_802_22_BS_Phy
{	
	unsigned int nSuperframeNumber:8; ///< Current superframe number	
	unsigned int nFrameNumber;	///< Current superframe number	
	...
	MODULATION_TECHNIQUE nModulation;
	...
	double dTTG;		///< Transmit/Receive Transition Gap. 105-333 micro sec. default 210 microsec
	double dDlUlRatio;
...
int fn_NetSim_CR_Metrics_F(char* file);
...
_declspec(dllexport) int fn_NetSim_CR_LoadPrimitivesDll(char* name,char* path,char* func);
