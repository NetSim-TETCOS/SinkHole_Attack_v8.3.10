...
int fn_NetSim_CR_CreateServiceFlow(NETSIM_ID nDeviceId,NETSIM_ID nInterfaceId,int nId,NetSim_PACKET* packet,double dTime)
{
		...
		pstruCPEMAC->pstruServiceParameter->Status[nId] == ServiceState_Deleted)
	{
		...
		pstruPacket->nDestinationId = pstruCPEMAC->nBSID;
		...
		pstruPacket->pstruMacData->dOverhead = DSX_IE_FIXED_SIZE + 3 + strlen(pstruDSAREQ->pstruIE.className);
		...
		pstruPacket->pstruMacData->szSourceMac = _strdup(DEVICE_HWADDRESS(nDeviceId,nInterfaceId));
...
int fn_NetSim_CR_BS_ProcessDSAReq()
{
	...
	pstruRSP->nDestinationId = pstruPacket->nTransmitterId;
...
int fn_NetSim_TerminateServiceFlow(NETSIM_ID nDeviceId, NETSIM_ID nInterfaceId, NetSim_PACKET* pstruPacket)
{
	...
	if(((GMH*)pstruPacket->pstruMacData->Packet_MACProtocol)->sz_Type[FRAGMANET_BIT] == '1')
	{
		...
		pstruDSD->nDestinationId = pstruCPEMAC->nBSID;
		...
		pstruDSD->pstruMacData->dOverhead = 7+strlen(pstruREQ->pstruIE.className)+DSX_IE_FIXED_SIZE;
...
int fn_NetSim_CR_BS_ProcessDSDReq()
{
	...
	pPacket->nDestinationId = pstruPacket->nSourceId;
	...
	pPacket->nSourceId = pstruPacket->nDestinationId;
	pPacket->nTransmitterId = pstruPacket->nReceiverId;
	pPacket->pstruMacData->dArrivalTime = pstruEventDetails->dEventTime;
	pPacket->pstruMacData->dOverhead = DSX_IE_FIXED_SIZE + 8 + strlen(pstruDSDRSP->pstruIE.className);
