...

{
	...
	{
		pstruEventDetails->dEventTime = pstruBSMAC->dSuperframeStartTime + pstruBSPhy->nFrameNumber*10000;
		pstruEventDetails->dPacketSize = 0;
...
int fn_NetSim_CR_CPE_ProcessFCH(NetSim_PACKET* pstruPacket)
{
	...
	if(pstruCPEPhy->nIntraFrameQuietPeriodLength)
	{
		if(pstruCPEPhy->szIntraFrameQuietPeriodBitmap[pstruCPEPhy->nFrameNumber-1] == '1')
		{
			...
			pstruEventDetails->dEventTime =pstruBSMAC->dFrameStartTime + (pstruFCH->n_Length_of_the_frame - pstruCPEPhy->nIntraFrameQuietPeriodDuration-1)*pstruSymbol->dSymbolDuration;
			pstruEventDetails->nEventType = TIMER_EVENT;
