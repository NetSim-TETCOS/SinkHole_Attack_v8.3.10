...
int fn_NetSim_CR_TransmitSCH()
{
	...
	char* szBSMAC = NETWORK->ppstruDeviceList[pstruEventDetails->nDeviceId-1]->ppstruInterfaceList[pstruEventDetails->nInterfaceId-1]->pstruMACLayer->szMacAddress;
	...
	pstruSCHPacket->nDestinationId = 0; //Broadcast packet
	...
	pstruSCHPacket->pstruMacData->szDestMac = _strdup(BROADCAST_MAC);
	pstruSCHPacket->pstruMacData->szSourceMac = _strdup(szBSMAC);
