...
int fn_NetSim_CR_QuietPeriod()
{
	...
	if(output->nSignalPresentVector == 0xFF)
	{
		...
		pstruPacket->pstruMacData->szDestMac = _strdup(DEVICE_HWADDRESS(pstruCPEMAC->nBSID,pstruCPEMAC->nBSInterface));
		pstruPacket->pstruMacData->szNextHopMac = _strdup(DEVICE_HWADDRESS(pstruCPEMAC->nBSID,pstruCPEMAC->nBSInterface));
		pstruPacket->pstruMacData->szSourceMac = _strdup(DEVICE_HWADDRESS(pstruEventDetails->nDeviceId,pstruEventDetails->nInterfaceId));
		pstruGMH->n_UCS = 1;
