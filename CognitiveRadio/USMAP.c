...
int fn_NetSim_CR_CPE_ProcessUSMAP()
{
	...
	dTime = pstruUSMAP->nAllocationStartTime*pstruSymbol->dSymbolDuration;
	dTime += pstruBSMac->dFrameStartTime;
	nSlot = (pstruUSMAP->nAllocationStartTime-1)*pstruSymbol->nOFDMSlots;
	...
	for(nLoop=0;nLoop<pstruUSMAP->nIECount;nLoop++)
	{
		if(pstruUSMAP->pstruUSMAPIE[nLoop]->nSID == pstruCPEMAC->nSID)
		{
			if(pstruUSMAP->pstruUSMAPIE[nLoop]->nUIUC >= 13 && pstruUSMAP->pstruUSMAPIE[nLoop]->nUIUC <= 61)
			{
				...
				pstruCPEMAC->BWRequestInfo->dStartTime = pstruBSMac->dFrameStartTime + nSlot*pstruSymbol->dSymbolDuration/pstruSymbol->nOFDMSlots;
			}
		}
		else
		{
