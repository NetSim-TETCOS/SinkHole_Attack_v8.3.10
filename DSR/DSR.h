...
#define MAINT_HOLD_OFF_TIME		(250*	MILLISECOND)
	...
	*/
	...
	/**	
	Packets are added to send buffer if device doesnt have route to target
	*/
	struct stru_DSR_SendBuffer
	{
		NETSIM_ID nDestId; ///< Destination IPAddress.
	...
	*/
	...
	int fn_NetSim_DSR_TransmitPacketFromSendBuffer(DSR_SEND_BUFFER* sendBuffer,
		NETSIM_ID nDeviceId,
		double dTime);
	int fn_NetSim_DSR_UpdateRouteCache(NETSIM_ID nDeviceId,
		unsigned int length,
		NETSIM_IPAddress* address,
		double dTime);
	DSR_ROUTE_CACHE* fn_NetSim_DSR_FindCache(DSR_DEVICE_VAR* devVar,NETSIM_IPAddress address,double dTime);
	...
	int fn_NetSim_DSR_ForwardRREP(NETSIM_ID nDeviceId,
	...
	NetSim_PACKET* fn_NetSim_DSR_InitRouteRequest(NETSIM_ID target,DSR_RREQ_TABLE** rreqTable,
		NETSIM_ID nDeviceId,
		double dTime,
		NetSim_EVENTDETAILS* pstruEventDetails);
...
#define DSR_EMPTY_MAINT_BUFFER(nDeviceId,nextHop) fn_NetSim_DSR_EmptyMaintBuffer(nDeviceId,nextHop)
	//Route cache
#define DSR_UPDATE_ROUTE_CACHE(nDeviceId,length,address,dTime) fn_NetSim_DSR_UpdateRouteCache(nDeviceId,length,address,dTime)
...
#define DSR_FORWARD_RREP(nDeviceId,packet) fn_NetSim_DSR_ForwardRREP(nDeviceId,packet,pstruEventDetails)
	//Route request
...
#define DSR_PROCESS_ACK()	fn_NetSim_DSR_ProcessAckOption(pstruEventDetails);
	...
	static NETSIM_IPAddress DSR_DEV_IP(NETSIM_ID dev)
	{
		if(dev)
			return fn_NetSim_Stack_GetFirstIPAddressAsId(dev,0);
		else
		{
			return fn_NetSim_Stack_GetFirstIPAddressAsId(dev,DEVICE_NWADDRESS(pstruEventDetails->nDeviceId,1)->type);
		}
	}
#define DSR_DEV_IP4(dev) fn_NetSim_Stack_GetFirstIPAddressAsId(dev,4)
#define DSR_DEV_IP6(dev) fn_NetSim_Stack_GetFirstIPAddressAsId(dev,6)
#define DSR_DEV_IP_COPY(dev) IP_COPY(DSR_DEV_IP(dev))
	//#define DSR_DEV_IP_COPY(dev) _strdup(DSR_DEV_IP(dev))
