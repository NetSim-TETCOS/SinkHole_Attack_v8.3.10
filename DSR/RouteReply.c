...
int fn_NetSim_DSR_GenerateRREP(NetSim_PACKET* rreqPacket,NetSim_EVENTDETAILS* pstruEventDetails)
{
	...
	rrep->Address[0]=DSR_DEV_IP_COPY(rreqPacket->nSourceId);
	rrep->Address[addressCount-1] = DSR_DEV_IP_COPY(pstruEventDetails->nDeviceId);
	...
	packet->pstruNetworkData->nTTL = addressCount+1;
...
int fn_NetSim_DSR_ProcessRREP(NetSim_EVENTDETAILS* pstruEventDetails)
{
	...
	DSR_UPDATE_ROUTE_CACHE(pstruEventDetails->nDeviceId,
		DSR_RREP_LEN(rrep),
		rrep->Address,
		pstruEventDetails->dEventTime);
	DSR_CHECK_SEND_BUFFER(pstruEventDetails->nDeviceId,
		pstruEventDetails->dEventTime);
	DSR_FORWARD_RREP(pstruEventDetails->nDeviceId,
	...
	NetSim_EVENTDETAILS* pstruEventDetails)
{
	...
	else
	{
		...
		rrep->nLastHopExternal = 0;
		rrep->nReserved = 0;
		addressCount = DSR_RREQ_LEN(rreq)+cache->nLength;
		...
		for(n=0;(unsigned int)nLoop<addressCount;nLoop++)
	...
	NetSim_EVENTDETAILS* pstruEventDetails)
{
	...
	if(!IP_COMPARE(rrep->Address[0],DSR_DEV_IP(nDeviceId)))
	{
	...
	while(n<length)
	{
		...
		if(!IP_COMPARE(rrep->Address[n],DSR_DEV_IP(nDeviceId)))
		{
