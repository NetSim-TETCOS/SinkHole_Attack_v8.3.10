...
struct stru_NetSim_Packet_TransportLayer
{
	...
	void* Packet_TransportProtocol; //Poinet to transport layer overhead
};
...
struct stru_NetSim_Packet_NetworkLayer
{
	...
	double dOverhead; //Overhead dur to network layer
...
struct stru_NetSim_Packet_MACLayer
{
	...
	NETSIM_MACAddress szSourceMac;//MAC address of source device
	NETSIM_MACAddress szDestMac;//MAC address of dest device
	NETSIM_MACAddress szNextHopMac;//MAC address of next hop
...
{
	int nPacketId; //Unique id of packet
	...
	NETSIM_ID nDestinationId; //Unique id of destination
...
_declspec(dllexport) PACKET_STATUS fn_NetSim_Packet_DecideError(int nBER, long double dPacketSize);
...
_declspec(dllexport) int fn_NetSim_Packet_CopyNWData(NetSim_PACKET* pstruDestPacket,const NetSim_PACKET* pstruSrcPacket,int,const char*);
...
//Used to get packet type based on application type
_declspec(dllexport) PACKET_TYPE fn_NetSim_Stack_GetPacketTypeBasedOnApplicationType(APPLICATION_TYPE nAppType);
