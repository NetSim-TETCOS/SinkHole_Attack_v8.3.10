...
enum dll_id
{
	...
	libZRP,
	libOLSR,
	nProtocolCount, //Keep this at last
};
...
_declspec(dllimport) struct stru_ProtocolInfo pstruProtocolInfo[nProtocolCount];
#else
#ifndef _NETSIM_PROTOCOL_INTERFACE_CODE_
extern struct stru_ProtocolInfo pstruProtocolInfo[nProtocolCount];
#endif
#endif
HINSTANCE hProtocolDll[nProtocolCount];
/*****************************************************************************
...
protocol_run fnpProtocol_Run[nProtocolCount];
...
protocol_init fnpProtocol_Init[nProtocolCount]; //Function name must be fn_NetSim_[Protocol Name]_init
//Function pointer for event trace
typedef char* (*protocol_trace)(int);	//Function name must be fn_NetSim_[Protocol Name]_trace
protocol_trace fnpProtocol_Trace[nProtocolCount];
...
protocol_CopyPacket fnpProtocol_CopyPacket[nProtocolCount];	//Function name must be fn_NetSim_[Protocol Name]_CopyPacket
//Function pointer for free packet
typedef int (*protocol_FreePacket)(const NetSim_PACKET* pstruPacket);
protocol_FreePacket fnpProtocol_FreePacket[nProtocolCount];	//Function name must be fn_NetSim_[Protocol Name]_FreePacket
//Function pointer for metrics
typedef int (*protocol_Metrics)(const char* szMetricsFile);
protocol_Metrics fnpProtocol_Metrics[nProtocolCount];		//Function name must be fn_NetSim_[Protocol Name]_Metrics
//Function pointer for finish
typedef int (*protocol_Finish)();
protocol_Finish fnpProtocol_Finish[nProtocolCount];		//Function name must be fn_NetSim_[Protocol Name]_Finish
//Function pointer for configure
typedef int (*protocol_Configure)(void**);
protocol_Configure fnpProtocol_Configure[nProtocolCount];		//Function name must be fn_NetSim_[Protocol Name]_Configure
//Function pointer to configure packet trace
typedef char* (*protocol_ConfigPacketTrace)(const void* xmlNetSimNode);
protocol_ConfigPacketTrace fnpProtocol_ConfigPacketTrace[nProtocolCount];	//Function name must be fn_NetSim_[Protocol Name]_ConfigPacketTrace
//Function pointer to write packet trace
typedef int (*protocol_WritePacketTrace)(const NetSim_PACKET*,char**);
protocol_WritePacketTrace fnpProtocol_WritePacketTrace[nProtocolCount];	//Function name must be fn_NetSim_[Protocol Name]_WritePacketTrace
