...
struct stru_NetSim_MacLayer
{
	...
	NETSIM_MACAddress szMacAddress; //MAC address of the device
...
struct stru_NetSim_EventDetails
{
	...
	int nPacketId; //Unique id for packet
	int nSegmentId;
...
//#define fn_NetSim_Utilities_GenerateRandomNo(x,y) fn_NetSim_Utilities_GenerateRandomNo(x,y,__FILE__,__LINE__)
//#endif
...
_declspec(dllexport) NETSIM_ID fn_NetSim_Stack_GetDeviceId_asMac(const char* mac,NETSIM_ID* nInterfaceId);
...
_declspec(dllexport) NETSIM_MACAddress fn_NetSim_Stack_GetMacAddressFromIP(const NETSIM_IPAddress szIPAddress);
//used to get the IP address from MAC address
_declspec(dllexport) NETSIM_IPAddress fn_NetSim_Stack_GetIPAddressFromMac(const NETSIM_MACAddress szMacAddress);
