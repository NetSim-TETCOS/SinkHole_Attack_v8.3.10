...
#ifndef NETSIM_ID
	typedef unsigned short int NETSIM_ID; //unique id
#endif 
typedef char* NETSIM_MACAddress;//MAC address
typedef char* NETSIM_Name;	//Unique name
typedef char* string;
typedef enum enum_Flag FLAG;
#ifndef __cplusplus
typedef enum enum_bool bool;
#endif
...
#define xmlError xmlErrorRedefine
# include "Stack.h"
# include "Packet.h"
#define fn_NetSim_LoadAppDllFunction(hsharedlib,sharedlibName,funName) fn_NetSim_LoadAppDllFunction_dbg(hsharedlib,sharedlibName,funName,__LINE__,__FILE__);
_declspec(dllexport) FARPROC fn_NetSim_LoadAppDllFunction_dbg(HINSTANCE* hdll,const char* dllName,const char* funName,int line,char* file);
...
_declspec(dllexport) int fn_NetSim_Anim_MoveNode(NETSIM_ID src,int x,int y,double time);
