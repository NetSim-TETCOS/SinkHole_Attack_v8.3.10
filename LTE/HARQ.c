...
int fn_NetSim_LTE_AddToHARQBuffer(NetSim_PACKET** HARQBuffer,NetSim_PACKET* packet,double dTime)
{
	unsigned int index=fn_NetSim_LTE_GetHARQIndex(dTime);
	NetSim_PACKET* list=HARQBuffer[index];
	if(!HARQBuffer[index])
	{
		HARQBuffer[index]=fn_NetSim_Packet_CopyPacket(packet);
	}
	else
	{
...
int fn_NetSim_LTE_SendAck(NetSim_PACKET* rlcSDU,NetSim_EVENTDETAILS* eventDetail)
{
	...
	NetSim_PACKET* packet=fn_NetSim_LTE_CreateCtrlPacket(eventDetail->dEventTime+4000,
		...
		rlcSDU->nTransmitterId,
		LTE_ACK_SIZE);
	LTE_ACK* ack=calloc(1,sizeof* ack);
...
int fn_NetSim_LTE_ENB_ProcessAck()
{
	...
	LTE_ASSOCIATEUE_INFO* info=fn_NetSim_LTE_FindInfo(DEVICE_MACVAR(pstruEventDetails->nDeviceId,pstruEventDetails->nInterfaceId),packet->nSourceId);
	if(info==NULL)
	{
		assert(false);
		return -1;
	}
	list=info->HARQBuffer[index];
	while(list)
	{
		...
		if(rlc->header->SN==ack->SN)
		{
			...
			if(temp==NULL)
			{
				info->HARQBuffer[index]=list->pstruNextPacket;
			}
			else
...
int fn_NetSim_LTE_UE_ProcessAck()
{
	...
	unsigned int index=fn_NetSim_LTE_GetHARQIndex(pstruEventDetails->dEventTime-4000);
	NetSim_PACKET* packet=pstruEventDetails->pPacket;
	LTE_UE_MAC* mac=DEVICE_MACVAR(pstruEventDetails->nDeviceId,pstruEventDetails->nInterfaceId);
	if(mac==NULL)
	{
		assert(false);
		return -1;
	}
	list=mac->HARQBuffer[index];
	while(list)
	{
		RLC_PDU* rlc=list->pstruMacData->Packet_MACProtocol;
		LTE_ACK* ack=packet->pstruMacData->Packet_MACProtocol;
		if(rlc->header->SN==ack->SN)
		{
			...
			if(temp==NULL)
			{
				mac->HARQBuffer[index]=list->pstruNextPacket;
			}
			else
				temp->pstruNextPacket=list->pstruNextPacket;
			list->pstruNextPacket=NULL;
...
int fn_NetSim_LTE_HARQ_RetransmitHARQBuffer_DL(LTE_ASSOCIATEUE_INFO* info,LTE_ENB_MAC* enbMac,NETSIM_ID enbId)
{
	unsigned int n=fn_NetSim_LTE_GetHARQIndex(pstruEventDetails->dEventTime);
	NetSim_PACKET* buffer=info->HARQBuffer[n];
	while(buffer)
	{
...
int fn_NetSim_LTE_HARQ_RetransmitHARQBuffer_UL(LTE_ASSOCIATEUE_INFO* info,LTE_ENB_MAC* enbMac,NETSIM_ID enbId)
{
	NETSIM_ID ueId=info->nUEId;
	NETSIM_ID ueInterface=info->nUEInterface;
	LTE_UE_MAC* mac=DEVICE_MACVAR(ueId,ueInterface);
	unsigned int n=fn_NetSim_LTE_GetHARQIndex(pstruEventDetails->dEventTime);
	NetSim_PACKET* buffer=mac->HARQBuffer[n];
	while(buffer)
	{
		unsigned int index=0;
		unsigned int size=(unsigned int)buffer->pstruMacData->dPayload;
	...
	//	if ((MODE_INDEX_MAPPING[info->TransmissionIndex.UL].TRANSMISSION_MODE == SingleUser_MIMO_Spacial_Multiplexing && info->ULInfo.modulation == Modulation_64QAM))	
		...
		while(index<LTE_NRB_MAX)
		{
			//if(size<TransportBlockSize1[nTBSIndex_DL][index])
			//	break;
			//index++;
			if ((MODE_INDEX_MAPPING[info->TransmissionIndex.UL].TRANSMISSION_MODE == SingleUser_MIMO_Spacial_Multiplexing && info->ULInfo.modulation== Modulation_64QAM)) 
