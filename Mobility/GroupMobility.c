...
int fn_NetSim_MoveGroup()
{
	...
	do
	{
		x=DEVICE_MOBILITY(group->nDevIds[0])->pstruCurrentPosition->X;
		y=DEVICE_MOBILITY(group->nDevIds[0])->pstruCurrentPosition->Y;
	...
	for(i=0;i<group->nDeviceCount;i++)
	{
		...
		fn_NetSim_Anim_MoveNode(dev,
			(int)DEVICE_POSITION(dev)->X,
			(int)DEVICE_POSITION(dev)->Y,
