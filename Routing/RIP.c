...
*/
_declspec(dllexport) int fn_NetSim_RIP_Metrics(char* szMetrics)
{
	return fn_NetSim_RIP_Metrics_F(szMetrics);	
}
/**
...
_declspec(dllexport)int fn_NetSim_RIP_Run()
{
	if(pstruEventDetails->pPacket && !fnCheckRoutingPacket())
		fnNetSimError("Unknown packet %d received in RIP protocol for Device %d.",pstruEventDetails->pPacket->nControlDataType,pstruEventDetails->nDeviceId);
