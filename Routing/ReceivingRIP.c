...
_declspec(dllexport) int fn_NetSim_RIP_ReceivingOf_RIP_Message(struct stru_NetSim_Network *pstruNETWORK,NetSim_EVENTDETAILS *pstruEventDetails)
{
	...
	((DEVICE_ROUTER*)(NETWORK->ppstruDeviceList[nDeviceId-1]->pstruApplicationLayer->routingVar))->uniInteriorRouting.struRIP.nStatus++;
	...
	while(pstruEntry!=NULL)
	{
		...
		pstruEventDetails->dEventTime=dEventTime+((DEVICE_ROUTER*)(NETWORK->ppstruDeviceList[nDeviceId-1]->pstruApplicationLayer->routingVar))->uniInteriorRouting.struRIP.n_timeout_timer;
		fnpAddEvent(pstruEventDetails);
