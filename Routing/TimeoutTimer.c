...
int fn_NetSim_RIP_Timeout_Timer(struct stru_NetSim_Network *pstruNETWORK,NetSim_EVENTDETAILS *pstruEventDetails)
{
	...
	while(pstruTempTable!=NULL)
	{
		...
		if(nRIP_UpdateVar==((DEVICE_ROUTER*)(NETWORK->ppstruDeviceList[pstruEventDetails->nDeviceId-1]->pstruApplicationLayer->routingVar))->uniInteriorRouting.struRIP.nStatus)
		{
			...
			pstruEventDetails->dEventTime=pstruEventDetails->dEventTime+((DEVICE_ROUTER*)(NETWORK->ppstruDeviceList[pstruEventDetails->nDeviceId-1]->pstruApplicationLayer->routingVar))->uniInteriorRouting.struRIP.n_garbage_collection_timer;
