...
int fn_NetSim_RIP_Update_Timer(struct stru_NetSim_Network *pstruNETWORK,NetSim_EVENTDETAILS *pstruEventDetails)
{
	...
	for(nLoop=0;nLoop<nInterfaceCount;nLoop++)
	{
			...
			NETWORK->ppstruDeviceList[nDeviceid-1]->ppstruInterfaceList[nLoop]->nInterfaceType==INTERFACE_WAN_ROUTER)
		{
			...
			if(!nLink_Id)
				...
				//if(pstruRouter && NETWORK->ppstruDeviceList[nConnectedDevId-1]->pstruApplicationLayer->nAppRoutingProtocol == APP_PROTOCOL_RIP)
				if(pstruRouter && pstruRouter->RoutingProtocol[nLoop] == APP_PROTOCOL_RIP)
				{
					...
					pstruRIPPacket->nVersion=((DEVICE_ROUTER*)(NETWORK->ppstruDeviceList[pstruEventDetails->nDeviceId-1]->pstruApplicationLayer->routingVar))->uniInteriorRouting.struRIP.n_RIP_Version;
					pstrudatabase=((DEVICE_ROUTER*)NETWORK->ppstruDeviceList[nDeviceid-1]->pstruApplicationLayer->routingVar)->pstruRoutingTables->pstruRIP_RoutingTable;
	...
	if(bWanPort)
	{
		...
		pstruEventDetails->dEventTime=pstruEventDetails->dEventTime+((DEVICE_ROUTER*)(NETWORK->ppstruDeviceList[pstruEventDetails->nDeviceId-1]->pstruApplicationLayer->routingVar))->uniInteriorRouting.struRIP.n_Update_timer;
		pstruEventDetails->pPacket = NULL;
		fnpAddEvent(pstruEventDetails);
	}
	return 0;
