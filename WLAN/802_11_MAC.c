...
int fn_NetSim_WLAN_PauseBackOff(NETSIM_ID nDeviceId, NETSIM_ID nInterfaceId, double dEventTime)
{
	...
	pstruMac->dMediumBusyTime = dEventTime;//pstruEventDetails->dEventTime;
...
int fn_NetSim_WLAN_StartTransmission()
{
	...
	{	
		...
		{
			pstruPacket = fn_NetSim_WLAN_Packet_GetAggregatedPacketFromBuffer(pstruMacVar->pstruQosList,pstruEventDetails->nDeviceId,1);
	...
	if(pstruPacket->nDestinationId) // Unicast packets
	{
		...
		case 1:// RTS-CTS Mechanism			
			...
			pstruEventDetails->nSubEventType = WLAN_SUBEVENT(SEND_RTS,nPortocolId);			
			break;
		default: // Basic Mechanism			
			fn_NetSim_WLAN_ChangeMacState(pstruMacVar,TXing_UNICAST);			
			pstruEventDetails->nSubEventType = WLAN_SUBEVENT(SEND_MPDU,nPortocolId);			
			break;
		}
	}
...
int fn_NetSim_WLAN_SendMPDU(double** dCummulativeReceivedPower)
{
	...
	if(pstruPacket->nDestinationId != 0) // Unicast packets
	{
		if(pstruPhyVar->nPHYprotocol != IEEE_802_11n && pstruPhyVar->nPHYprotocol != IEEE_802_11ac)
		{
			...
			pstruEventDetails->nSubEventType = WLAN_SUBEVENT(ACK_TIMEOUT,nProtocolId);
		}
		else
		{
			pstruPacket=pstruEventDetails->pPacket;
		//	while(pstruPacket)
		//	{
		//		dPacketSize += fnGetPacketSize(pstruPacket);
		//		pstruPacket=pstruPacket->pstruNextPacket;
		//	}
			if(pstruPhyVar->nPHYprotocol==IEEE_802_11n)
			{
				...
				dPacketSize = 32;//.0;//(double) Block_ACK_SIZE;
			...
			else if(pstruPhyVar->nPHYprotocol==IEEE_802_11ac)
			{
				...
				dPacketSize = 152;//.0;//(double) Block_ACK_SIZE;
			...
			dAckTimeout = pstruMacVar->dTransmissionTime \
				+ pstruPhyVar->pstruPhyChar->naSIFSTime \
				+ dAckTxtime + 1;	
			pstruEventDetails->nSubEventType = WLAN_SUBEVENT(BLOCK_ACK_TIMEOUT,nProtocolId);
		}
		// Change the state
		pstruPacket=pstruEventDetails->pPacket;
	...
	else // broadcast packet	
	{
		...
		//	{
		//		dPacketSize = fnGetPacketSize(pstruPacket);
		//		pstruMacVar->dTransmissionTime = fn_NetSim_WLAN_802_11n_TxTimeCalculation(dPacketSize,1,nDeviceId,nInterfaceId);
		//		dPacketSize = 14;//.0;//(double) ACK_SIZE;
		//		dAckTxtime = fn_NetSim_WLAN_802_11n_TxTimeCalculation(dPacketSize,0,nDeviceId,nInterfaceId);
		//		dAckTimeout = pstruMacVar->dTransmissionTime \
		//			+ pstruPhyVar->pstruPhyChar->naSIFSTime \
		//			+ dAckTxtime + 1;		
		//	}
		//	// Change the state
		//	fn_NetSim_WLAN_ChangeMacState(pstruMacVar,Wait_ACK);
		//	// Add ACK Timeout event 
		//	pstruEventDetails->dEventTime = dTime + dAckTimeout;	
		//	pstruEventDetails->nSubEventType = WLAN_SUBEVENT(ACK_TIMEOUT,nProtocolId);
		//	pstruEventDetails->nEventType = TIMER_EVENT;
...
int fn_NetSim_WLAN_Add_MAC_Header(NETSIM_ID nDeviceId,NETSIM_ID nInterfaceId, NetSim_PACKET *pstruPacket)
{
	...
	case LinkType_P2MP:
		...
		{	
			...
			pstruMacHeader->Address1 = _strdup(pstruPhyVar->szBSSID);
			pstruMacHeader->Address2 = _strdup(pstruPacket->pstruMacData->szSourceMac);//_strdup(DEVICE_HWADDRESS(nDeviceId, nInterfaceId));
			if(pstruPacket->nDestinationId != 0) // unicast packets
				pstruMacHeader->Address3 = _strdup(pstruPacket->pstruMacData->szDestMac);
			else
		...
		else
		{
			...
			if(pstruPacket->nDestinationId != 0)
				pstruMacHeader->Address1 = _strdup(pstruPacket->pstruMacData->szDestMac);
			else
				pstruMacHeader->Address1  = NULL;
			pstruMacHeader->Address2 = _strdup(pstruPhyVar->szBSSID);
			pstruMacHeader->Address3 = _strdup(pstruPacket->pstruMacData->szSourceMac);			
	...
	case LinkType_MP2MP:
			...
			if(pstruPacket->nDestinationId != 0) // Unicast Frames;
				...
				pstruMacHeader->Address1  = NULL;
			pstruMacHeader->Address2 = _strdup(pstruPacket->pstruMacData->szSourceMac);	 
			//pstruMacHeader->Address3 = _strdup(pstruPhyVar->szBSSID);		not needed
...
int fn_NetSim_WLAN_SendACK()
{
	...
	pstruAck->RA =_strdup( ((MAC_HEADER*)(pstruPacket->pstruMacData->Packet_MACProtocol))->Address2 );
...
int fn_NetSim_WLAN_SendBlockACK()
{
	...
	BlockACK_FRAME *pstruAck = pstruEventDetails->pPacket->pstruMacData->Packet_MACProtocol;
...
int fn_NetSim_WLAN_SendRTS(double** dCummulativeReceivedPower)
{
	...
	pstruRts->TA = _strdup(NETWORK->ppstruDeviceList[nDeviceId-1]->ppstruInterfaceList[nInterfaceId-1]->pstruMACLayer->szMacAddress); //_strdup(DEVICE_HWADDRESS(nDeviceId,nInterfaceId));
	if(nDeviceId != pstruPhyVar->nBSSID ) 
	{
		pstruRts->RA = _strdup(pstruPhyVar->szBSSID);
	...
	{
		pstruRts->RA = _strdup(pstruPacket->pstruMacData->szDestMac);
	...
	pstruEventDetails->nSubEventType = WLAN_SUBEVENT(CTS_TIMEOUT,pstruEventDetails->nProtocolId);	
	pstruEventDetails->nEventType = TIMER_EVENT;
...
int fn_NetSim_WLAN_SendCTS()
{
	...
	pstruCts =  fnpAllocateMemory(1, sizeof(CTS_FRAME));
	pstruCts->pstruFrameControl = fnpAllocateMemory(1,sizeof(FRAME_CONTROL));	
	pstruCts->pstruFrameControl->FrameType = CONTROL;
	pstruCts->pstruFrameControl->usnSubType = CTS;
	pstruCts->RA =_strdup(((RTS_FRAME*)(pstruPacket->pstruMacData->Packet_MACProtocol))->TA);
	if(nDeviceId != pstruPhyVar->nBSSID )// && pstruPhyVar->BssType == INFRASTRUCTURE)
	{		
	...
	//pstruCtsPkt->nDestinationId = pstruPacket->nSourceId; 
	pstruCtsPkt->nDestinationId = 0; //0; // Broadcast CTS
	...
	pstruCtsPkt->nPacketId = 0;//(unsigned int)pstruEventDetails->nPacketId ;
...
int fn_NetSim_WLAN_ReceiveMPDU()
{
	...
	while(pstruPacket)
	{
		...
		if(pstruPhyVar->nPHYprotocol == IEEE_802_11n||pstruPhyVar->nPHYprotocol == IEEE_802_11ac) 
			{
			//	pstruPacket->pstruMacData->dPayload=pstruPacket->pstruPhyData->dPacketSize-MAC_OVERHEAD_802_11n;
		...
		case INFRASTRUCTURE:
			...
			case 0: // broadcast packet			
				...
				else // For AP
				{
					...
					//pstruEventDetails->nSubEventType = WLAN_SUBEVENT(SEND_ACK,pstruEventDetails->nProtocolId);
					//pstruEventDetails->nEventType = MAC_OUT_EVENT;
					//fnpAddEvent(pstruEventDetails);	
			...
			default: // Unicast packet
				...
				{		
					...
					else if(pstruPhyVar->nPHYprotocol==IEEE_802_11n || pstruPhyVar->nPHYprotocol==IEEE_802_11ac)
					{
						...
						//If first Pcket is errored, delete all packets and send Error 
						//Message
						if(pstruPacketHead->nPacketStatus==PacketStatus_Error)
						{
							Aggregated_Packet_FreePacket(pstruPacketHead);
							//Add MAC_OUT event to send ACK	that all Packets are errored		
							pstruEventDetails->dEventTime = dTime + pstruPhyVar->pstruPhyChar->naSIFSTime;			
							pstruEventDetails->pPacket = pstruBlockAckPkt;			
							pstruEventDetails->nSubEventType = WLAN_SUBEVENT(SEND_BLOCK_ACK,pstruEventDetails->nProtocolId);
							pstruEventDetails->nEventType = MAC_OUT_EVENT;
							fnpAddEvent(pstruEventDetails);	
							fn_NetSim_WLAN_ChangeMacState(pstruMacVar,TXing_ACK);
							return 0;
		...
		case INDEPENDENT:
			...
			else
			{
				...
				else if(pstruPhyVar->nPHYprotocol==IEEE_802_11n||pstruPhyVar->nPHYprotocol==IEEE_802_11ac)
				{
					...
					//If first Pcket is errored, delete all packets and send Error 
					//Message
					pstruPacketHead=pstruEventDetails->pPacket;
					if(pstruPacketHead->nPacketStatus==PacketStatus_Error)
					{
						Aggregated_Packet_FreePacket(pstruPacketHead);
						//Add MAC_OUT event to send ACK	that all Packets are errored		
...
int fn_NetSim_WLAN_ReceiveACK()
{
	...
	if(!strcmp(pstruAck->RA,DEVICE_HWADDRESS(nDeviceId,nInterfaceId)))
	{	
...
int fn_NetSim_WLAN_ReceiveRTS()
{
	...
	if(!strcmp(pstruRts->RA,DEVICE_HWADDRESS(nDeviceId,nInterfaceId)))
	{
		//if(pstruMacVar->dNAV > pstruEventDetails->dEventTime)
		//{
		//	pstruMacVar->pstruMetrics->nRTSFailureDueToNav++;
		...
		pstruEventDetails->dEventTime = dTime + pstruPhyVar->pstruPhyChar->naSIFSTime;		
		pstruEventDetails->nEventType = MAC_OUT_EVENT;
		pstruEventDetails->nSubEventType = WLAN_SUBEVENT(SEND_CTS,nProtocolId);	
		fnpAddEvent(pstruEventDetails);	
		// Change the State
	...
	{	
		//fn_NetSim_WLAN_ChangeNAV(pstruRts->nDuration);
		if(pstruMacVar->dNAV > pstruEventDetails->dEventTime)
		{
...
int fn_NetSim_WLAN_ReceiveCTS()
{
	...
	PHY_VAR *pstruPhyVar = DEVICE_PHYVAR(nDeviceId,nInterfaceId); 
	NetSim_PACKET *pstruPacket = pstruEventDetails->pPacket;
	CTS_FRAME *pstruCts = pstruPacket->pstruMacData->Packet_MACProtocol;

	if(!strcmp(pstruCts->RA,DEVICE_HWADDRESS(nDeviceId,nInterfaceId)))
	{
		...
		pstruEventDetails->dEventTime = dTime + pstruPhyVar->pstruPhyChar->naSIFSTime;		
		pstruEventDetails->nEventType = MAC_OUT_EVENT;
		pstruEventDetails->nSubEventType = WLAN_SUBEVENT(SEND_MPDU,nProtocolId);	
		fnpAddEvent(pstruEventDetails);
		// Change the State
		fn_NetSim_WLAN_ChangeMacState(pstruMacVar,TXing_UNICAST);
	...
	{
		//fn_NetSim_WLAN_ChangeNAV(pstruCts->nDuration);
		if(pstruMacVar->dNAV > pstruEventDetails->dEventTime)
		{
...
int fn_NetSim_WLAN_AckTimeOut()
{
	...
	case 1: //Retransmit the frame	
		...
		if(pstruMacVar->pstruTempData)
		{
			//pstruPacket = fn_NetSim_Packet_CopyAggregatedPacket(pstruMacVar->pstruTempData);				
			pstruPacket = pstruMacVar->pstruTempData;
			while(pstruPacket)
			{
				fn_NetSim_WLAN_FreePacket(pstruPacket);					
	...
	case 0: // Drop the frame			
		...
		if(pstruMacVar->pstruTempData)
		{
			...
			if(pstruPacket->DropNotification)
				pstruPacket->DropNotification(pstruPacket);
			//fn_NetSim_Packet_FreeAPacket(pstruMacVar->pstruTempData);
		...
		{		
			...
			for(nLoop = 1;nLoop<=NETWORK->ppstruDeviceList[nDeviceId-1]->nNumOfInterface;nLoop++)
			{
				if(nLoop == nInterfaceId)
					continue;
				//pstruPacketCopy = pstruPacket;//fn_NetSim_Packet_CopyAggregatedPacket(pstruPacket);
				//fn_NetSim_WLAN_FreePacket(pstruPacketCopy); //Free the MAC Data	
...
int fn_NetSim_WLAN_CheckPacketsInPacketlist(NETSIM_ID nDevId, NETSIM_ID nInterfaceId)
{
	...
	if(pstruPacket->pstruNetworkData)
		...
		pstruEventDetails->dPacketSize = pstruPacket->pstruMacData->dPacketSize;
	pstruEventDetails->pPacket = NULL;//pstruPacket;
