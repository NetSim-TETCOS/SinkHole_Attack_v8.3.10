...
#include "main.h"
#include "WLAN.h"
int fn_NetSim_WLAN_RandomNoShadowLoss(double *ldRandNo,unsigned long *uSeed,unsigned long *uSeed1);
/**
...
_declspec (dllexport) int fn_NetSim_WLAN_CalculateShadowLoss(unsigned long ulSeed1, unsigned long ulSeed2, double* dShadowReceivedPower,double dStandardDeviation)
{
	...
	if(nIset==0)
	{
		...
		{		
			...
			fn_NetSim_WLAN_RandomNoShadowLoss(&ldRandomNumber,&ulSeed1,&ulSeed2);		
...
*/
int fn_NetSim_WLAN_CalculateBER(NETSIM_ID nDeviceId, NETSIM_ID nInterfaceId, NetSim_PACKET *pstruPacket)
{
	...
	if(pstruPhy->ChannelModel == NO_PATHLOSS)
	{
		...
		pstruPhy->nErrorRate = 0;//Assign Error Range value
		return 0;
	}
	switch(pstruPhy->PhyType)
	{
	case DSSS:	
		fn_NetSim_WLAN_802_11b_BER(nDeviceId,nInterfaceId,pstruPacket);
		return 0;
		break;
	case OFDM:	
		fn_NetSim_WLAN_802_11a_11g_BER(nDeviceId,nInterfaceId,pstruPacket);
		return 0;
		break; 
	case HT:
		fn_NetSim_WLAN_802_11n_BER(nDeviceId,nInterfaceId,pstruPacket);
		return 0;
		break;	
	case VHT:
		fn_NetSim_WLAN_802_11ac_BER(nDeviceId,nInterfaceId,pstruPacket);
		return 0;
		break;
	default:
						...
						for(njInterface = 1; njInterface <= NETWORK->ppstruDeviceList[nj-1]->nNumOfInterface; njInterface++)
						{
							...
							case PHY_MEDIUM_WIRELESS:
								...
								else
								{
									...
									{
										dCummulativeReceivedPower[ni][nj] = 10*log10(pstruPhyj->nTxPower);
										if(i==0)
											dCummulativeReceivedPower2[ni][nj] = 10*log10(pstruPhyj->nTxPower);
									}
									else// pathloss 
									{						
								...
								break;
							default:
								dCummulativeReceivedPower[ni][nj] = 0;
								if(i==0)
									dCummulativeReceivedPower2[ni][nj] = 0;
								break;
							}
						}//jInterface				
					}
...
/**
Transmit the packet in the Mediun, i.e from Physical out to Physical In.
...
{
	PHY_VAR *pstruPhy = NETWORK->ppstruDeviceList[nConDevId-1]->ppstruInterfaceList[nConInterface -1]->pstruPhysicalLayer->phyVar;
	PHY_VAR *pstruTxPhy = NETWORK->ppstruDeviceList[nDevId-1]->ppstruInterfaceList[nInterfaceId -1]->pstruPhysicalLayer->phyVar;
	MAC_VAR *pstruTxMac;	
	...
	//This if condition adds sub PHYin Event eith nSubEventType = AMPDU_SUBFRAME//
		//*Adding SubPhyIn Event, i.e. AMPDU_SUBFRAME
	if(SubEventType==AMPDU_SUBFRAME && (pstruPhy->nPHYprotocol == IEEE_802_11n || pstruPhy->nPHYprotocol == IEEE_802_11ac) && pstruPacket->nDestinationId!=0)
	{
	...
	if( (pstruPhy->RadioState == RX_ON_IDLE) && (dCummulativeReceivedPower[nDevId][nConDevId] > pstruTxPhy->dMinReceiverSensitivity))
	{
		...
		//Add PHYSICAL_IN_EVENT
		//if(pstruPhy->nPHYprotocol!=IEEE_802_11n)
		
		//To get an End Time to add Physical In Event
		while(pstruPacketList->pstruNextPacket!=NULL)
		{
			pstruPacketList=pstruPacketList->pstruNextPacket;
				//To get appropriate end time
		}
		...
		pevent.nPacketId = pstruPacket->nPacketId;		
		pevent.nProtocolId = fn_NetSim_Stack_GetMacProtocol(nConDevId,nConInterface);
		if((pstruPhy->nPHYprotocol==IEEE_802_11n||pstruPhy->nPHYprotocol==IEEE_802_11ac) && pstruPacket->nDestinationId!=0 )
		{				
	...
	{
		pstruPacketList=pstruPacket;
		pstruTxMac = NETWORK->ppstruDeviceList[nDevId-1]->ppstruInterfaceList[nInterfaceId-1]->pstruMACLayer->MacVar;
		...
		while(pstruPacketHead)
		{
			...
			WLAN_CHANGERADIOSTATE(nDevId,nInterfaceId,RX_ON_IDLE);// 0 - CHANNEL IDLE
