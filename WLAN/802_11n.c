...
Intialize time parameters as per the Table 20-5—Timing-related constants 802.11n-2009 pdf 
also intialize 802.11n PHY header relevent parameters.
*/
...
*/
int fn_NetSim_WLAN_802_11n_BER(NETSIM_ID nDeviceId,NETSIM_ID nInterfaceId,NetSim_PACKET *pstruPacket)
{
	double dBer = 0.0;
	double dBer1 = 0.0;
	int nCount = 1;
	...
	ldEbByN0 = pstruPhy->dTotalReceivedPower + ldLoss - ldNpower-30;//bit energy calculation.30dB is subtracted from the energy to convert the value from dBm to dB
	...
	pstruPhy->nErrorRate = nCount-1;//Assign Error Range value
	return 0;
}
