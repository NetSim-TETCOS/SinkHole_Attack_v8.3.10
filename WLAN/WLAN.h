 ...
 enum enum_802_11_Subevent_Type
{
 ...
 #define WLAN_SUBEVENT(subEvent,Protocol) Protocol*100+subEvent 
...
struct stru_802_11_MAC_Frame_Header 
{
	...
	NETSIM_MACAddress Address1;				//	6 bytes  
	NETSIM_MACAddress Address2;				//	6 bytes  
	NETSIM_MACAddress Address3;				//	6 bytes  
	...
	NETSIM_MACAddress Address4;				//	6 bytes  
...
struct stru_802_11_RTS_Frame
{
	...
	NETSIM_MACAddress RA;							// 6 bytes
	NETSIM_MACAddress TA;							// 6 bytes
	int nFCS;										// 4 bytes	
	// Simulation parameters
	double dBOTime;
};
...
struct stru_802_11_BlockACK_Frame
{
	...
	//bool BitMapac[1024];							//128 bytes = 1024 bits
	int nFCS;										// 4 bytes
													// 32bytes total
};
...
//#define Calculate_BER(snr,bertable) calculate_ber(snr,bertable,sizeof(bertable)/sizeof(BER))
