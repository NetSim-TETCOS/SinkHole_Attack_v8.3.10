...
int fn_NetSim_BRP_Update(IP_ROUTINGTABLE* iarpTable)
{
	...
	while(iarpTable)
	{
		...
		iarpTable=(IP_ROUTINGTABLE*)LIST_NEXT(iarpTable);
	}
	return 0;
}
		...
		DEVICE_NWADDRESS(pstruEventDetails->nDeviceId,1)))
	{
		...
		while(tree)
		{
			if(IP_COMPARE(tree->borderIP,header->PrevBordercastAddress))
			{
				...
				BRP_HEADER* temp=(BRP_HEADER*)calloc(1,sizeof* temp);
				...
				packet->nDestinationId = tree->boderId;
