...
IP_ROUTINGTABLE* findInIarpTable(IP_ROUTINGTABLE* table,NETSIM_IPAddress ip)
{
	while(table)
	{
...
int addToIERPTableFromIARP(NODE_IERP* ierp,IP_ROUTINGTABLE* iarpTable)
{
	IP_ROUTINGTABLE* table= iarpTable;
	while(iarpTable)
	{
		IP_ROUTINGTABLE* temp = iarpTable;
		unsigned int i;
		...
		iarpTable=(IP_ROUTINGTABLE*)LIST_NEXT(temp);
	}
	return 0;
}
...
int flushIERPTableFromIARP(NODE_IERP* ierp,IP_ROUTINGTABLE* iarpTable)
{
	IERP_ROUTE_TABLE* table = ierp->routeTable;
	IP_ROUTINGTABLE* temp;
	while(table)
	{
		if(table->flag)
		{
			...
			while(temp)
			{
				if(!IP_COMPARE(temp->networkDestination,nextHop) && temp->Metric==1)
					break;
				temp=(IP_ROUTINGTABLE*)LIST_NEXT(temp);
			}
			if(!temp)
			{
