	...
	case MAC_IN_EVENT:
		{
			...
			if(WSN_MAC_HEADER(pstruPacket)->nAckRequestFlag)
			{
				...
				pstruAckPacket->nDestinationId = pstruAckPacket->nReceiverId;
				...
				pstruAckPacket->pstruMacData->szSourceMac = _strdup(pstruPacket->pstruMacData->szDestMac);
				pstruAckPacket->pstruMacData->szDestMac = _strdup(pstruPacket->pstruMacData->szSourceMac);
	...
	case PHYSICAL_OUT_EVENT:
		{
			...
			NETSIM_ID nLoop;
			NetSim_PACKET* pstruPacket;
			NETSIM_ID nConnectionID=0;			/*Device id which is connected*/
			NETSIM_ID nConnectionPortID=0;		/* Connected device's port no which is connected with the current system*/
			...
			nLink_Id=fn_NetSim_Stack_GetConnectedDevice(nDevice_Id,nDevice_PortId,&nConnectionID,&nConnectionPortID);
			...
			{
				if(nLoop == pstruPacket->nTransmitterId)
					continue;

				WSN_PHY(nLoop)->dTotalReceivedPower += dCummulativeReceivedPower[pstruPacket->nTransmitterId-1][nLoop-1];

				if(nLoop==pstruPacket->nReceiverId || pstruPacket->nReceiverId == 0)
				{
					...
					if(WSN_PHY(nLoop)->dTotalReceivedPower - dCummulativeReceivedPower[pstruPacket->nTransmitterId-1][nLoop-1] >= WSN_PHY(nLoop)->dReceiverSensivity)
						packet->nPacketStatus = PacketStatus_Collided;

					if(MW_DBM(dCummulativeReceivedPower[pstruPacket->nTransmitterId-1][nLoop-1]) > MW_DBM(WSN_PHY(nLoop)->dReceiverSensivity)*2)
					{
						if(WSN_PHY(nLoop)->nRadioState == RX_ON_IDLE)
						{

							if(!ZIGBEE_CHANGERADIOSTATE(nLoop, WSN_PHY(nLoop)->nRadioState, RX_ON_BUSY))
								return 0;
							// Metrics
							WSN_MAC(nLoop)->nNodeStatus = RX_MODE;
							txFlag=true;
							...
							pstruEventDetails->nInterfaceId = fn_NetSim_Stack_GetWirelessInterface(nLink_Id,nLoop);
	...
	case PHYSICAL_IN_EVENT: 
		{
			...
			if(WSN_PHY(pstruEventDetails->nDeviceId)->dTotalReceivedPower - dCummulativeReceivedPower[pstruPacket->nTransmitterId-1][pstruPacket->nReceiverId-1] >= WSN_PHY(pstruEventDetails->nDeviceId)->dReceiverSensivity)
				pstruPacket->nPacketStatus = PacketStatus_Collided;


			nPacketStatus = pstruPacket->nPacketStatus;

			ZIGBEE_SINR(&SNR, WSN_PHY(pstruEventDetails->nDeviceId)->dTotalReceivedPower,
				dCummulativeReceivedPower[pstruPacket->nTransmitterId-1][pstruPacket->nReceiverId-1]);
			...
			case UPDATE_MEDIUM:
				{
					...
					for(nLoop=1; nLoop<=NETWORK->ppstruNetSimLinks[nLink_Id-1]->puniDevList.pstruMP2MP.nConnectedDeviceCount; nLoop++)
					{
						if(nLoop != pstruEventDetails->nDeviceId)
						{
							WSN_PHY(nLoop)->dTotalReceivedPower -= dCummulativeReceivedPower[nTransmitterID-1][nLoop-1];

							if(WSN_PHY(nLoop)->dTotalReceivedPower < WSN_PHY(nLoop)->dReceiverSensivity)
								WSN_PHY(nLoop)->dTotalReceivedPower = 0;
						}
					}
			...
			case BEACON_TRANSMISSION_END:
				{
					...
					for(nLoop=0;nLoop<NETWORK->nDeviceCount;nLoop++)
					{
						...
						{
							WSN_MAC(nLoop+1)->nNodeStatus = RX_ON_IDLE;
			...
			case CFP_END:
				{
					...
					//Set the superframe status as inactive
...
/**  This function Initializes the Zibee protocol parameters.*/
...
int fn_NetSim_Zigbee_ChannelFormation(CHANNELS** pstruChannelList,NetSim_EVENTDETAILS* pstruEventDetails)
{
	*pstruChannelList = calloc(1,sizeof(CHANNELS));
	...
	(*pstruChannelList)->dFrequency = NETWORK->ppstruNetSimLinks[0]->puniMedProp.pstruWirelessLink.dFrequency;
