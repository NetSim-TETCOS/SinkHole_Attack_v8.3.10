...
#define DBM_MW(dbm) (pow(10,dbm/10))
#define MW_DBM(mw) (10*log10(mw))
...
#define MAINLINE 2
	...
	typedef enum enum_RadioState RADIO_STATE;
	...
	/** Enumeration for Zigbee Radio state */
	enum enum_RadioState
	{
		RX_ON_IDLE,
		SLEEP,
		TRX_ON_BUSY,
	...
	struct stru_Agent
	{
		...
		int nMobilityModel;
		/// Stroes X-coordiante of the environment.
		double dWorkingArea_X;
		/// Stores Y-coordiante of the environment.
	...
	/// Datastructure for IEEE 802.15.4 metrics which consists of sensor metrics and power model metrics also.
	struct stru_Metrics
	{
...
#define ZIGBEE_SENSOREVENT(nSensorLoop) fn_NetSim_Zigbee_SensorEvent(nSensorLoop,nGlobalPANCoordinatorId,pstruAgent,pstru_Sensor,pstruMetrics,pstruEventDetails)
...
#define ZIGBEE_SINR(SNR,dTotalReceivedPower,dReceivedPower) fn_NetSim_Zigbee_CalculateSINR(SNR,dTotalReceivedPower,dReceivedPower,pstruEventDetails)
	...
	_declspec(dllexport) int fn_NetSim_Zigbee_Metrics(char *);
	int fn_NetSim_Zigbee_Metrics_F(char *);
...
#define AGENTLOG_ALLOC() (AGENTLOG*)list_alloc(sizeof(AGENTLOG),offsetof(AGENTLOG,ele))
	...
	int fn_NetSim_Zigbee_SensorEvent(int nSensorLoop,NETSIM_ID nGlobalPANCoordinatorId,AGENT** pstruAgent,SENSORS* pstru_Sensor,METRICS** pstruMetrics,NetSim_EVENTDETAILS* pstruEventDetails);
