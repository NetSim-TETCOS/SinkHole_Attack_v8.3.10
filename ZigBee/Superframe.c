...
int fn_NetSim_Zigbee_SuperFrameInitialization(SUPERFRAME** ppstruSuperFrame,NETSIM_ID nGlobalPANCoordinatorId,METRICS** pstruMetrics,NetSim_EVENTDETAILS* pstruEventDetails)
{
	...
	pstruSuperFrame->pstruBeacon->dPayload = 5 + 3*pstruSuperFrame->nNumberOfGTS;
	...
	pstruPacket->pstruMacData->szSourceMac = _strdup(fn_NetSim_Stack_GetMacAddressFromIP(fn_NetSim_Stack_GetFirstIPAddressAsId(pstruEventDetails->nDeviceId,0)));
	pstruPacket->pstruMacData->szDestMac = _strdup("FFFFFFFFFFFF");
